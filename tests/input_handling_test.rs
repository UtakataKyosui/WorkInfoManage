...
impl Storage for MockStorage {
    async fn load_tasks(&self) -> Result<Vec<tasks::Model>> { Ok(vec![]) }
    async fn save_tasks(&self, _: &[tasks::Model]) -> Result<()> { Ok(()) }
...
impl ReportStorage for MockReportStorage {
    async fn load_report(&self, _: NaiveDate) -> Result<Option<DailyReport>> { Ok(None) }
    async fn save_report(&self, _: &DailyReport) -> Result<()> { Ok(()) }
    async fn delete_report(&self, _: NaiveDate) -> Result<()> { Ok(()) }
    async fn list_report_dates(&self, _: i32, _: u32) -> Result<Vec<NaiveDate>> { Ok(vec![]) }
}
