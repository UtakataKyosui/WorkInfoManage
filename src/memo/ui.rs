...
pub fn draw_memo_list(app: &mut App, frame: &mut Frame) {
    ...
    let tree = Tree::new(&tree_items)
        ...
        .experimental_scrollbar(Some(ratatui::widgets::Scrollbar::new(
    ...
    let preview_text: Vec<Line> = if let Some(selected_id) = memo_state.tree_state.selected().last()
    {
        ...
        if let Some(memo) = memo_state
            .memos
            .iter()
            .find(|m| m.id == path_str || m.path.to_string_lossy() == path_str)
        {
            memo.content
                .lines()
                .map(|line| {
...
pub fn draw_memo_edit(app: &mut App, frame: &mut Frame) {
    ...
    memo_state
        ...
        .set_block(Block::default().borders(Borders::ALL).title(
            if memo_state.editing_memo_path.is_some() {
                "Edit Memo"
            } else {
                "New Memo"
            },
    ...
    let help_text =
        "Esc: 保存終了 | Ctrl+s: 保存 | Ctrl+c: キャンセル | Ctrl+v: 貼付 | Alt+c: コピー";
    let help = Paragraph::new(help_text)
        .block(Block::default().borders(Borders::ALL))
        .style(Style::default().fg(Color::Gray));
