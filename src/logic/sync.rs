    ...
    pub fn parse_asana_tasks(&self, data: &Value, log: &mut std::fs::File) -> Result<Vec<tasks::Model>> {
        ...
        if let Some(tasks_array) = data.get("data").and_then(|v| v.as_array()) {
            for task_obj in tasks_array {
                ...
                let notes_str = task_obj.get("notes")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                
                // Parse task type from resource_subtype
                let task_type = task_obj.get("resource_subtype")
                    .and_then(|v| v.as_str())
        ...
        let check_reviewer = |username: &str| -> ReviewerState {
            ...
            if let Some(review) = user_review {
                if let Some(state) = review.get("state").and_then(|s| s.as_str()) {
                    ...
                    url = review.get("html_url").and_then(|u| u.as_str()).map(String::from);
                }
            }
            ...
            .open("logs/sync_detail.log")
        {
            ...
            Err(e) => {
                ...
                std::fs::OpenOptions::new()
                    .write(true)
                    .open("/dev/null")
                    .unwrap_or_else(|_| std::fs::File::create("/tmp/taskmanager_dummy.log").unwrap())
            }
        };
