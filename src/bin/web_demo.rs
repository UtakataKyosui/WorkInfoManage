    ...
    async fn load_tasks(&self) -> Result<Vec<tasks::Model>> {
        ...
        Ok(vec![
            ...
            tasks::Model {
                ...
                review_status: Some(
                    serde_json::json!({"status": "pending", "reviewers": ["reviewer1"]}),
                ),
                last_updated_at: chrono::Local::now().naive_local(),
            },
        ])
    }
    async fn save_tasks(&self, _tasks: &[tasks::Model]) -> Result<()> {
        Ok(())
    }
    async fn load_notes(&self, task_id: i32) -> Result<Vec<task_notes::Model>> {
        // Return dummy notes for demo
        if task_id == 1 {
            Ok(vec![task_notes::Model {
                id: 1,
                task_id: 1,
...
impl ReportStorage for WebReportStorage {
    ...
    async fn delete_report(&self, _date: NaiveDate) -> Result<()> {
        Ok(())
    }
}
        ...
        if app.input_mode {
            ...
            match app.current_screen {
                ...
                CurrentScreen::UnifiedMemoList => {
                    work_info_manage::input::InputHandler::handle_unified_memo_list(
                        &mut app, key_event,
                    );
                }
                _ => {
